...
![](https://leiniao-ibg.feishu.cn/space/api/box/stream/download/asynccode/?code=NmQxN2Y1OGMwZDlkNjAyZDY5MzI3MjE5Mzk2ZTE3ZWZfUTBOeWFQQjhSQllJSjJnWmpzMVdxV1ZPTFg1NFNuVEdfVG9rZW46WTl4UWJQa1Rnb01QV254N3Frc2N0Y1BtbmFjXzE3NzQyNTI3MTA6MTc3NDI1NjMxMF9WNA)
...
![](https://leiniao-ibg.feishu.cn/space/api/box/stream/download/asynccode/?code=Yjc1MWRhN2FlZTE4ZWI5OWQxYzE2YTU3OTk2ZjgxODlfc2VCTXpVeTQ3Mk1HSjlVbHk1NDliQVRZRzl4UHRKanBfVG9rZW46UUZsY2I3M2VRbzRJaTV4bzlaRGNlb1dPbjRkXzE3NzQyNTI3MTA6MTc3NDI1NjMxMF9WNA)

注：Fragment+RecyclerView合目的场景可以参考sample中的**FragmentRecyclerViewDemo**
...
MirrorContainerView的使用可参考MirrorContainerViewActivity，注意一定要实现**mirrorContainer.bindTo**方法，否则合目不生效；
...
![](https://leiniao-ibg.feishu.cn/space/api/box/stream/download/asynccode/?code=OWE2NWMyOGUxY2U0MGI0NjVhNGE5MmY0NzdmNDgyMzZfV3pWSU82TkxqelhwRjFWYTM4WVF2OWNuRXJ3eklobjFfVG9rZW46R1lNTmJwN216bzgxUlF4NEhnVmNTQmtObmJlXzE3NzQyNTI3MTA6MTc3NDI1NjMxMF9WNA)
...
并且在CommondTouchBack中，增加了属性filterMode,当CommonTouchCallback设置为_OnlyX的时候onTPSlideContinuous只返回X轴滑动数据，_当CommonTouchCallback设置为_OnlyY的时候，onTPSlideContinuous只返回Y轴滑动的数据。__**只有RayneoX3支持此项设置**__。_

并且TempleAction.TpSlideContinuous也增加了滑动的方向是否是X轴还是Y轴。

2. ### SlideBackward & SlideForward
...
![](https://leiniao-ibg.feishu.cn/space/api/box/stream/download/asynccode/?code=YWM4MWRhNzJjYTdjZmE4YmIzMTFkM2NjZDkwYzA1OTJfSENGdnlFeFU2eWc2NE0ydHhvRWtpTmtJSjlIU1ViM2VfVG9rZW46V1QzeGJqYW1hb25RQ1Z4dHV0ZWNSTmJhbmdoXzE3NzQyNTI3MTA6MTc3NDI1NjMxMF9WNA)
...
![](https://leiniao-ibg.feishu.cn/space/api/box/stream/download/asynccode/?code=YzE5Yjk2YTM5N2ZiNmQ0OThhNjc1YjcxNWZjMWIzMjZfWHNYQUViS0ttUld3VVphWHpTZGxZZFNEVlc3eW42UkRfVG9rZW46S3lLamIzeWFJb3l1b2h4bEFnSWNHWXlrbnhmXzE3NzQyNTI3MTA6MTc3NDI1NjMxMF9WNA)
...
**continuous ：**是否为连续滑动模式？默认为false，也就是单步滑动模式，每一次滑动，只会触发一次焦点的next()/previous()，当置为true的时候，会根据滑动的总距离来切换焦点，可以连续切换多个焦点的next()/previous()

**isVertical：**是否只接受垂直方向的滑动来切换焦点，默认为true，如果设置为false，就只默认接受水平方向的滑动来切换焦点

**ignoreDelta：**焦点切换的滑动距离阀值，只有当滑动距离超过这个阀值才会触发焦点的切换逻辑
...
|RECORD_TRANSLATION|[device.id](http://device.id) == SPEAKER_MIC ;CHANNEL_IN_MONO;  <br>送翻译引擎的采样率一般设置16k;|ExampleRecordTranslation.java|调用前麦克风，录制周围人声，适合会议记录/现场翻译|
|CAMCORDER|[device.id](http://device.id) == SPEAKER_MIC;  <br>CHANNEL_IN_STEREO;  <br>采样率根据需要设定，最高不超出48kHz;|ExampleRecordCamcorder.java|左右镜腿麦克风，立体声录音，全向收音，适合录像场景使用|
|VOICE_RECOGNITION|[device.id](http://device.id) == SPEAKER_MIC;  <br>CHANNEL_IN_MONO;  <br>识别引擎一般要求Sample rate为16kHz，单通道;|ExampleRecordRecognition.java|调用三颗麦克风，只收佩戴者声音，送识别引擎识别，不考虑音质，适合于语音助手，导航场景等|
|VOICE_COMMUNICATION|[device.id](http://device.id) == SPEAKER_MIC|ExampleRecordCommunication.java|调用前麦和右方麦克风，只收佩戴者的声音，适用于网络通话、网络会议等|
...
snapdragonCamera的源码示例（AE收敛状态=2则可以正常取图）： [https://github.com/Lh1600852534/snapdragonCamera/blob/master/src/com/android/camera/CaptureModule.java](https://github.com/Lh1600852534/snapdragonCamera/blob/master/src/com/android/camera/CaptureModule.java)
